...
struct waitqueue* jobselect()
{
	...
	if (head) {
		for (prev = head, p = head; p != NULL; prev = p, p = p->next) {
			...
			if (p->job->curpri > highest || (p->job->curpri == highest && p->job->wait_time > longer_waiting)) {
				select = p;
				selectprev = prev;
				highest = p->job->curpri;
				longer_waiting = p->job->wait_time;
			}
		}		
...
void do_deq(struct jobcmd deqcmd)
{
	...
	if (current && current->job->jid == deqid) {
		...
		if (select) {
			printf("terminate job in waitqueue: %d\n", select->job->jid);
